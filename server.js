...
app.post('/api/admin/products', requireAuth, upload.array('images', 10), async (req, res) => {
  const { name, description, price, category, stock, sizes, sizes_stock, shipping_days, compare_price, brand_id, hot, low_stock } = req.body || {};
  ...
  const isLowStock = low_stock === '1' || low_stock === 'true' || low_stock === true ? 1 : 0;

  try {
    const result = db.prepare(
      'INSERT INTO products (name, description, price, category, stock, sizes, sizes_stock, shipping_days, compare_price, brand_id, hot, low_stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(
      ...
      shipDays, compPrice, brandId, isHot, isLowStock
    );
...
app.put('/api/admin/products/:id', requireAuth, upload.array('images', 10), async (req, res) => {
  ...
  const { name, description, price, category, stock, sizes, sizes_stock, shipping_days, compare_price, remove_image_ids, brand_id, hot, low_stock } = req.body || {};
  ...
  try {
    ...
    db.prepare(
      `UPDATE products SET name = ?, description = ?, price = ?, category = ?, stock = ?, sizes = ?,
       sizes_stock = ?, shipping_days = ?, compare_price = ?, brand_id = ?, hot = ?, low_stock = ?, updated_at = datetime('now') WHERE id = ?`
    ).run(
      name.trim(), (description || '').trim(), Number(price),
      category, totalStock,
      JSON.stringify(Array.isArray(parsedSizes) ? parsedSizes : []),
